 ...
 */
angular.module('kobaApp').controller("CalendarController",
  ['$scope', '$window', 'kobaFactory',
    function ($scope, $window, kobaFactory) {
      ...
      function init() {
        ...
        $scope.openingHours = drupalSettings.koba_booking.opening_hours;
        ...
        $scope.$watchGroup(['selected.date', 'selected.resource'],
          function (val) {
            ...
            // Convert from sunday as zero index to monday as zero index.
            var weekday = ($scope.selected.date.day() + 6) % 7;
            $scope.interestPeriod.start = $scope.openingHours[weekday].from;
            $scope.interestPeriod.end = $scope.openingHours[weekday].to;
       ...
       */
      function getSelectedDateTimesAsUnixTimestamp() {
